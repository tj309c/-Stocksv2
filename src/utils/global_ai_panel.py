...
XAI_API_KEY = "xai-..."
        ...
        st.info(f"**{len(available_models)}/{len(AI_MODELS)}** AI models ready")
        ...
        with st.spinner("🧠 AI models are analyzing... This may take 30-60 seconds..."):
            ...
            try:
                result = loop.run_until_complete(
                    analyzer.analyze_with_all_models(aggregated_data, selected_models)
                )
            finally:
                loop.close()
    ...
    if 'ai_analysis_result' in st.session_state:
        ...
        st.caption(f"Generated: {analysis_time.strftime('%Y-%m-%d %H:%M:%S')} | " + 
                  f"Models: {', '.join([AI_MODELS[m].name for m in result['successful_models']])}")
...
def render_detailed_breakdown(result: Dict):
    ...
    with tabs[3]:
        if 'risk_assessment' in response:
            ...
            with st.expander("Black Swan Scenarios"):
