...
def show_crypto_dashboard(components, ticker="BTC-USD"):
    ...
    <div style="text-align: center; padding: 20px 0;">
        <h1 style="margin: 0; font-size: 3em;">🚀 CRYPTO Dashboard</h1>
        <p style="color: #888; font-size: 1.2em; margin: 10px 0;"><i>When Lambo? When Moon?</i> 🌙</p>
    </div>
...
def show_fear_greed_tab(data):
    ...
    if not df.empty and len(df) >= 30:
        ...
        fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=score,
            title={'text': f"{emoji} {label}"},
            domain={'x': [0, 1], 'y': [0, 1]},
            gauge={
                'axis': {'range': [0, 100]},
                'bar': {'color': color},
                'steps': [
                    {'range': [0, 25], 'color': "#FF3860"},
                    {'range': [25, 45], 'color': "#FF9560"},
                    {'range': [45, 55], 'color': "#FFB700"},
                    {'range': [55, 75], 'color': "#7FFF7F"},
